...
test.describe('time tracking lock race flow', () => {
  test.skip(!hasAdminE2ECredentials(), 'Set admin or superadmin credentials to run the time-tracking lock race flow.');

  test('employee gets a backend lock conflict when an admin locks the month after the week was already open', async ({ browser, request }) => {
    ...
    const costCenter = await createCostCenter(adminApi, {
      code: `E2L-${suffix}`.slice(0, 20),
      name: `E2E Lock Race ${suffix}`,
      description: 'Generated for time-tracking lock race flow',
    });
    let lockedMonth: { year: number; month: number } | null = null;
    ...
    try {
      ...
      try {
        ...
        lockedMonth = {
          ...
          month: targetDateObject.getMonth() + 1,
        };
        ...
        try {
          ...
          await setTimeLock(adminApi, { ...lockedMonth, locked: true });

          await page.getByRole('button', { name: /save all/i }).click();
          await expect(snackBar).toContainText('This date is locked and cannot be edited.');
      ...
      if (lockedMonth !== null) {
        await setTimeLock(adminApi, { ...lockedMonth, locked: false });
      }
