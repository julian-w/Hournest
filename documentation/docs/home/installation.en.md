...
- intentionally writes `APP_KEY` into `.env` via `php artisan key:generate`
- does **not** automatically write the superadmin hash into `.env`, but requires an explicit operator decision
